...
RSpec.describe "PirateShipsCrewIndex", type: :feature do
  before do
    ...
    @crew_member_4 = CrewMember.create!(pirate_ship_id: @ship_2.id, name: "Billy Bob", age: 27, role: "Swashbuckler", active: false)
  end
  ...
  it "User views the list of crew members for a pirate ship" do
    ...
    expect(page).not_to have_content(@crew_member_4.name)
    expect(page).not_to have_content(@crew_member_4.age)
    expect(page).not_to have_content(@crew_member_4.role)
    expect(page).not_to have_content(@crew_member_4.active)
  end

  it "doesn't show inactive crew members" do
    visit "/pirate_ships/#{@ship.id}/crew_members"
